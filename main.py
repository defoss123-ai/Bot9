...
async def main() -> None:
    ...
    async def start_trading(api_key: str, api_secret: str) -> bool:
        try:
            ...
            logger.info("Подключение к MEXC Spot успешно")
            return True
    ...
    if not api_key or not api_secret:
        logger.warning("API ключи не найдены. Запускаем веб-интерфейс и ждём ввода...")
    else:
        await start_trading(api_key, api_secret)
    ...
    if context.trader is None:
        async def wait_for_keys() -> None:
            while context.running and context.trader is None:
                key, secret = await encrypted_settings.get_api_keys()
                if key and secret:
                    logger.info("Ключи появились, создаём exchange и трейдера")
                    ok = await start_trading(key, secret)
                    if ok:
                        web.trader = context.trader
                        break
                await asyncio.sleep(5)

        context.tasks.append(asyncio.create_task(wait_for_keys()))
