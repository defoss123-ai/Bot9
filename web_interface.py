...
class WebInterface:
    ...
    def register_routes(self):
        ...
        async def save_pair():
            ...
            enabled = enabled_raw in (True, "true", "on", "1", 1)
            ...
            await self.context.db.execute(
                """
                INSERT OR REPLACE INTO pairs (symbol, enabled, tp_percent, sl_percent, cancel_time)
                VALUES (?, ?, ?, ?, ?)
                """,
                (symbol, 1 if enabled else 0, tp_percent, sl_percent, cancel_time),
            )
        ...
        async def test_connection():
            ...
            try:
                ...
                return jsonify({"success": True, "message": "Подключение к спотовому API успешно"})
            ...
            return jsonify(
                {
                    ...
                    "risk_per_trade": float(await self.config_manager.get("risk_per_trade", 10.0)),
                }
            )
        ...
        async def save_strategy():
            ...
            await self.config_manager.set("risk_per_trade", float(data.get("risk_per_trade", 10.0)))
            return jsonify({"success": True})
